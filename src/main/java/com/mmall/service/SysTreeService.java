...
public class SysTreeService {
    ...
    //level:0.0
    public void transformDeptTree(List<DeptLevelDto>deptLevelDtoList,String level, Multimap<String,DeptLevelDto> levelDepMap){
        for(int i=0;i<deptLevelDtoList.size();i++){
            ...
            //处理下一层
            List<DeptLevelDto> tempDeptList =(List<DeptLevelDto>) levelDepMap.get(nextLevel);
            //使用的是那个包？
            if(!CollectionUtils.isEmpty(tempDeptList)){
                ...
                //进入到下一层处理
                transformDeptTree(tempDeptList,nextLevel,levelDepMap);
            }
        }
    }
